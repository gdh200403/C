...
public:
    AssemblyError(const std::string& message)
...
enum CommandType { OPERATION, PSEUDO, ZERO };
...
static std::string FormatLine(const std::string &line) {
    ...
    // check if the line contains .STRINGZ
    if (result.find(".STRINGZ") != std::string::npos) {
        // remove the leading and trailing whitespace chars
        Trim(result);
        return result;
    }
...
static int RecognizeNumberValue(const std::string &str) {
    ...
    if (str.substr(0, 1) == "X") {
        ...
        try{
            ...
            throw AssemblyError("Label does not exist: " + str);
        }
    } else if (str.substr(0, 1) == "B") {
        // 8进制
        try{
            return std::stoi(str.substr(1), nullptr, 8);
        }catch(...){
            throw AssemblyError("Label does not exist: " + str);
        }
...
static std::string ConvertBin2Hex(const std::string &bin) {
    ...
    std::stringstream ss;
    ss << std::hex << std::setw(4) << std::setfill('0') << std::bitset<16>(bin).to_ulong();
    return ss.str();
}
...
private:
    ...
    std::string TranslateOprand(unsigned int current_address, std::string str,
                                unsigned int line_number, int opcode_length = 3
                                );
